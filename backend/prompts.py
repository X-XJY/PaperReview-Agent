...
输出必须是符合给定 schema 的 JSON 对象，不输出 Markdown 围栏。缺失事实用 null 或空数组。
...
PROMPTS = {
 ...
 'extract': BASE + '提取本文具体方法和步骤、带条件的优势、作者明确承认的局限、作者未来工作、数据集及评价指标。不得从实验缺失推断作者局限。局限未找到返回空数组。evaluation 保留划分、指标条件，value 不明确则 null。',
